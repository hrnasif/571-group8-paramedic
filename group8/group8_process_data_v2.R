...
set.seed(123)
rel_subset <- c(1:9, sample(16:ncol(relative), 20, replace = F))
relative_small <- relative[,rel_subset]
...
subjs <- c(2,14,15,18)
group8_qpcr_data_v2 <- absolute %>% filter(subject_id %in% subjs & sample_id <= 10)
group8_16S_data_v2 <- relative_small %>% filter(subject_id %in% subjs & sample_id <= 10)

save(group8_qpcr_data_v2, file = "data/group8_qpcr_data_v2.rda")
save(group8_16S_data_v2, file = "data/group8_16S_data_v2.rda")
